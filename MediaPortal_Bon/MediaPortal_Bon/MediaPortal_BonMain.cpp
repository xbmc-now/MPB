...
CMediaPortal_BonMain::CMediaPortal_BonMain(void)
{
	this->msgWnd = NULL;
	this->nwCtrlID = 0;
...
void CMediaPortal_BonMain::SetHwnd(HWND wnd)
{
	this->msgWnd = wnd;
}
...
DWORD CMediaPortal_BonMain::EnumBonDriver(
...
DWORD CMediaPortal_BonMain::OpenBonDriver(
...
//戻り値：
// エラーコード
DWORD CMediaPortal_BonMain::CloseBonDriver()
{
	this->currentBonDriver = L"";
...
DWORD CMediaPortal_BonMain::GetServiceList(
...
BOOL CMediaPortal_BonMain::GetOpenBonDriver(
...
DWORD CMediaPortal_BonMain::SetCh(
	WORD ONID,
	WORD TSID,
	WORD SID
	)
...
void CMediaPortal_BonMain::GetCh(
	WORD* ONID,
	WORD* TSID,
	WORD* SID
	)
...
BOOL CMediaPortal_BonMain::GetCh(
	DWORD* space,
	DWORD* ch
	)
{
...
//戻り値：
// TRUE（変更中）、FALSE（完了）
BOOL CMediaPortal_BonMain::IsChChanging(BOOL* chChgErr)
{
	return this->bonCtrl.IsChChanging(chChgErr);
}

void CMediaPortal_BonMain::SetSID(
	WORD SID
	)
{
	this->lastSID = SID;
...
//引数：
// enableFlag		[IN]TRUE（開始）、FALSE（停止）
BOOL CMediaPortal_BonMain::SendTCP(
	BOOL enableFlag
	)
{
...
//UDP送信の設定数を取得
DWORD CMediaPortal_BonMain::GetCountUDP()
{
	return this->udpCount;
}

//TCP送信の設定数を取得
DWORD CMediaPortal_BonMain::GetCountTCP()
{
	return this->tcpCount;
}

//UDPの送信中先一覧取得
BOOL CMediaPortal_BonMain::GetSendUDPList(
...
//TCPの送信中先一覧取得
BOOL CMediaPortal_BonMain::GetSendTCPList(
	vector<NW_SEND_INFO>* sendList
	)
{
...
DWORD CMediaPortal_BonMain::GetEpgInfo(
	BOOL nextFlag,
	CString* epgInfo
	)
{
...
//戻り値：
// シグナルレベル
float CMediaPortal_BonMain::GetSignalLevel()
{
	return this->bonCtrl.GetSignalLevel();
}

//エラーカウントをクリアする
void CMediaPortal_BonMain::ClearErrCount(
	)
{
	if( this->nwCtrlID != 0 ){
		this->bonCtrl.ClearErrCount(this->nwCtrlID);
...
void CMediaPortal_BonMain::GetErrCount(
	ULONGLONG* drop,
	ULONGLONG* scramble
	)
{
...
//戻り値：
// TRUE（成功）、FALSE（失敗）
BOOL CMediaPortal_BonMain::StartRec(
	)
{
...
//録画を停止する
// TRUE（成功）、FALSE（失敗）
BOOL CMediaPortal_BonMain::StopRec()
{
	if( this->recCtrlID != 0 ){
...
BOOL CMediaPortal_BonMain::IsRec()
{
	return this->bonCtrl.IsRec();
}

//予約録画を停止する
void CMediaPortal_BonMain::StopReserveRec()
{
...
//戻り値：
// エラーコード
DWORD CMediaPortal_BonMain::StartChScan()
{
...
//戻り値：
// エラーコード
DWORD CMediaPortal_BonMain::StopChScan()
{
...
DWORD CMediaPortal_BonMain::GetChScanStatus(
	DWORD* space,
	DWORD* ch,
	wstring* chName,
	DWORD* chkNum,
...
//戻り値：
// エラーコード
DWORD CMediaPortal_BonMain::StartEpgCap(
	)
{
...
//戻り値：
// エラーコード
DWORD CMediaPortal_BonMain::StopEpgCap(
	)
{
...
//引数：
// info			[OUT]取得中のサービス
DWORD CMediaPortal_BonMain::GetEpgCapStatus(
...
//Viewアプリの起動を行う
void CMediaPortal_BonMain::ViewAppOpen()
{
...
void CMediaPortal_BonMain::StartServer()
{
	wstring pipeName = L"";
	wstring eventName = L"";
...
void CMediaPortal_BonMain::StopServer()
{
	this->pipeServer.StopServer();
}

void CMediaPortal_BonMain::StartTimeShift()
{
	wstring saveFile = L"";
	DWORD ctrlID = 0;
	if( this->recCtrlID != 0 ){
...
BOOL CMediaPortal_BonMain::GetViewStatusInfo(
	float* signal,
	DWORD* space,
	DWORD* ch,
	ULONGLONG* drop,
...
int CALLBACK CMediaPortal_BonMain::CtrlCmdCallback(void* param, CMD_STREAM* cmdParam, CMD_STREAM* resParam)
{
	CMediaPortal_BonMain* sys = (CMediaPortal_BonMain*)param;
...
void CMediaPortal_BonMain::GetMpLogPath(
	CString& szValue
	)
{
