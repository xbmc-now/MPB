...
void CTstOpnMain::SetHwnd(HWND wnd)
{
	this->msgWnd = wnd;
}

//設定を行う
void CTstOpnMain::ReloadSetting()
{
...
DWORD CTstOpnMain::EnumBonDriver(
...
DWORD CTstOpnMain::OpenBonDriver(
...
//戻り値：
// エラーコード
DWORD CTstOpnMain::CloseBonDriver()
{
	this->currentBonDriver = L"";
...
DWORD CTstOpnMain::GetServiceList(
...
BOOL CTstOpnMain::GetOpenBonDriver(
...
//戻り値：
// TRUE（変更中）、FALSE（完了）
BOOL CTstOpnMain::IsChChanging(BOOL* chChgErr)
{
	return this->bonCtrl.IsChChanging(chChgErr);
}

void CTstOpnMain::SetSID(
	WORD SID
	)
{
	this->lastSID = SID;
...
/*			if( this->sendUdpFlag == FALSE && this->sendTcpFlag == FALSE ){
		...
		if( this->nwCtrlID != 0 ){
			if( this->allService == TRUE ){
				...
				this->bonCtrl.SetServiceID(this->nwCtrlID, this->lastSID);
				_tprintf(_T("nwCtrlID:%d lastSID:%d\r\n"), this->nwCtrlID, this->lastSID);
			}
		}
		this->bonCtrl.SetScramble(this->nwCtrlID, this->enableScrambleFlag);
		this->bonCtrl.SetServiceMode(this->nwCtrlID, this->needCaption, this->needData);
...
//UDP送信の設定数を取得
DWORD CTstOpnMain::GetCountUDP()
{
	return this->udpCount;
}

//TCP送信の設定数を取得
DWORD CTstOpnMain::GetCountTCP()
{
	return this->tcpCount;
}

//UDPの送信中先一覧取得
BOOL CTstOpnMain::GetSendUDPList(
...
//TCPの送信中先一覧取得
BOOL CTstOpnMain::GetSendTCPList(
	vector<NW_SEND_INFO>* sendList
	)
{
...
//戻り値：
// シグナルレベル
float CTstOpnMain::GetSignalLevel()
{
	return this->bonCtrl.GetSignalLevel();
}

//エラーカウントをクリアする
void CTstOpnMain::ClearErrCount(
	)
{
	if( this->nwCtrlID != 0 ){
		this->bonCtrl.ClearErrCount(this->nwCtrlID);
...
BOOL CTstOpnMain::IsRec()
{
	return this->bonCtrl.IsRec();
}

//予約録画を停止する
void CTstOpnMain::StopReserveRec()
{
...
DWORD CTstOpnMain::GetChScanStatus(
	DWORD* space,
	DWORD* ch,
	wstring* chName,
	DWORD* chkNum,
...
//引数：
// info			[OUT]取得中のサービス
DWORD CTstOpnMain::GetEpgCapStatus(
...
void CTstOpnMain::StopServer()
{
	this->pipeServer.StopServer();
}

void CTstOpnMain::StartTimeShift()
{
	wstring saveFile = L"";
	DWORD ctrlID = 0;
	if( this->recCtrlID != 0 ){
...
BOOL CTstOpnMain::GetViewStatusInfo(
	float* signal,
	DWORD* space,
	DWORD* ch,
	ULONGLONG* drop,
...
int CALLBACK CTstOpnMain::CtrlCmdCallback(void* param, CMD_STREAM* cmdParam, CMD_STREAM* resParam)
{
	CTstOpnMain* sys = (CTstOpnMain*)param;
